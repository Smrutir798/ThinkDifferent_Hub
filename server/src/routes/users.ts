...
router.post('/', auth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    await dbService.activities.create(
      `Administrator registered new user account '${newUser.name}' (${newUser.role}) in the system database.`,
      'system'
    );
...
router.put('/:id', auth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Create activity event log
    await dbService.activities.create(
      `Administrator updated user account details for '${updatedUser.name}' (${updatedUser.role}).`,
      'system'
    );
...
router.put('/:id/reset-password', auth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Create activity event log
    await dbService.activities.create(
      `Administrator reset the password for user account '${user.name}'.`,
      'system'
    );
...
router.delete('/:id', auth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Create activity event log
    await dbService.activities.create(
      `Administrator deleted user account '${user.name}' (${user.role}) from system.`,
      'system'
    );
